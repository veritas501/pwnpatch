...
class Patch(object):
    ...
    def hook(self,vaddr,asm=None,c=None,byte=None):
        ...
        if byte is not None:
            ...
            self.patch(usr_code_addr,byte=byte+'\xc3')#在后面放一个ret
            '''
            思路：
            ...
            if self.arch == 1: # i386
                ...
                patch_bytes1 = origin_code[:first_inst_len] + pwn.asm('jmp $+%d'%((shellcode_addr+2*patch_byte_len+0x2a)-(vaddr+first_inst_len))).ljust(5,'\x90')
                patch_bytes2 = pwn.asm('jmp $+%d'%(shellcode_addr+2*patch_byte_len-(vaddr))).ljust(5,'\x90')+origin_code[5:]
...
class Segment(object):
    ...
    def _find_cave(self,length):
        ...
        if code_end != None and data_start != None and data_start-code_end >= length:
            ...
            if len(self.mem) > (segment_end-self.load_addr):# 还是由于64两个段分的很开的原因
                self.mem.ljust(((len(self.mem)&~0xfff)+0x1000)+length,'\x00')
            else:
                self.mem.ljust(segment_end-self.load_addr+length,'\x00')
    ...
    def add_segments(self,addr,length=0x1000,prot=7,pht_size=0x400):
        ...
        self.mem[len(self.mem):] = '\x00'*length

        self.header.e_phnum+=1
        self.mem[self.header.e_phoff:] = self.dump_segments().ljust(pht_size,'\x00')
