...
class PePatcher(BasePatcher):
    def __init__(self, filename: str, need_detail=True, minimal_edit=False):
        self.fast_load = not need_detail

        with open(filename, "rb") as f:
            self._pefile: PE = pefile.PE(data=f.read(), fast_load=self.fast_load)
    ...
    def _init_basic_info(self):
        # 初始化基础信息
        self.exe_type = enums.EXE.PE
        machine = self._pefile.NT_HEADERS.FILE_HEADER.Machine
        self.arch = {
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_I386"]: enums.Arch.I386,
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_AMD64"]: enums.Arch.AMD64,
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_ARM"]: enums.Arch.ARM,
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_ARM64"]: enums.Arch.ARM64,
        }.get(machine, enums.Arch.NONE)
        ...
        self.endian = enums.Endian.LSB
        self.bit_size = {
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_I386"]: enums.BitSize.X32,
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_AMD64"]: enums.BitSize.X64,
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_ARM"]: enums.BitSize.X32,
            pefile.MACHINE_TYPE["IMAGE_FILE_MACHINE_ARM64"]: enums.BitSize.X64,
        }.get(machine, enums.BitSize.NONE)
    ...
    def _init_code_cave(self) -> bool:
        ...
        for i, section in enumerate(self._pefile.sections):
            ...
            for flag_name in target_flags:
                target_characteristics_mask |= pefile.SECTION_CHARACTERISTICS[flag_name]

            # flags 不满足条件，跳过下一个
            if (
                characteristics & target_characteristics_mask
                != target_characteristics_mask
            ):
                continue
    ...
    def _reload(self, fast_load=True):
        self._pefile = pefile.PE(data=self._pefile.write(), fast_load=fast_load)
    ...
    def _has_room_for_new_section_header(self) -> bool:
        ...
        now_header_end = (
            self._pefile.sections[-1].get_file_offset() + section_header_size
        )
    ...
    def _fix_image_size(self):
        ...
        for section in self._pefile.sections:
            # 计算区段的舍入后大小
            virtual_size = self.align(section.Misc_VirtualSize, section_alignment)

            # 更新映像的大小
            size_of_image = max(size_of_image, section.VirtualAddress + virtual_size)
    ...
    def _alloc_room_for_new_section_header(self) -> bool:
        ...
        new_max_header_addr = self.align(
            max_header_addr + section_header_size, alignment
        )
        ...
        dst_secs = []
        for section in sorted(self._pefile.sections, key=lambda s: s.PointerToRawData):
        ...
        ret = [mod_buf[:max_header_addr], bytes(new_max_header_addr - max_header_addr)]
        for sec in dst_secs:
            ret.append(sec.src_buf)
            ret.append(sec.dst_padding)

        self._pefile = pefile.PE(data=b"".join(ret), fast_load=self.fast_load)
    ...
    def _add_segment(self, addr: int = 0, length: int = 0x1000, prot: int = 7) -> int:
        ...
        if prot & enums.SegProt.R.value:
            target_flags.append("IMAGE_SCN_MEM_READ")
        if prot & enums.SegProt.W.value:
            target_flags.append("IMAGE_SCN_MEM_WRITE")
        if prot & enums.SegProt.X.value:
            target_flags.append("IMAGE_SCN_MEM_EXECUTE")
        target_characteristics = 0
        for flag_name in target_flags:
            target_characteristics |= pefile.SECTION_CHARACTERISTICS[flag_name]
        ...
        self._set_file_bytes(
            new_section.PointerToRawData, bytes(new_section.SizeOfRawData)
        )
...
""".format(vaddr, hex(vaddr), backup_length, origin2_jmp_addr)
    ...
    def add_segment(self, addr: int = 0, length: int = 0x1000, prot: int = 7) -> int:
        ...
        if new_segment:
            log.success(
                "Add segment @ 0x{:x}, size: 0x{:x}".format(new_segment, length)
            )
        return new_segment
